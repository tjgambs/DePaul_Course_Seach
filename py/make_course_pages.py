...
def create_webpage(filename):
	with open(filename,'r') as class_data:
		...
		html += '''<link rel="shortcut icon" href="../other/icon.png"><link rel="stylesheet" href="../css/stylesheet.css" type="text/css" media="print, projection, screen" />
					...
					<div class="right" class="inner"><h3><a href="../course-cart.html" style="text-decoration:none">Course Cart</a></h3></div><body>'''
		html += '<h1>' + title + '</h1>' + course_description + '<h2>Available Classes</h2><table id="myTable" class="tablesorter">'
		html += '<thead><tr>'
		...
		for i in info[3:]:
			...
			if(i[0] != '0.0' and os.path.exists('../teachers/' + teacher_file + '.html')):
				html+='<tr>'
				html+='<td>' + '<a href = "../teachers/' + teacher_file + '.html" style="text-decoration:none">' + i[0] + '</a></td>'
				html+='<td>' + i[1] + '</td>'
				html+='<td>' + i[2] + '</td>'
				html+='<td>' + '<a href = "../teachers/' + teacher_file + '.html" style="text-decoration:none">' + i[3] + '</a></td>'
				html+='<td>' + '<a href = "../teachers/' + teacher_file + '.html" style="text-decoration:none">' + i[4] + '</a></td>'
