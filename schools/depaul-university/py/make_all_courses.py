...
def create_page(full_name,description,course_url):
    ...
    <link rel="stylesheet" id="tipue3" type="text/css" href="../../default/tipuesearch/tipuesearch.css">
	<script>
...
<h1>'''
	...
	with open('../'+webpage_name,'w') as output:
			output.write(html)
	return [full_name,description,'classes/'+'-'.join(full_name.split()[:2]).replace(';','').lower()+'.html']
